<template>
    ...
    <TableView v-model:tableOption="tableConfig" @handelPagination="handelPagination">
        ...
        <template #handleBtn="{row}">
            ...
            <BtnAction v-model:rowDto="roleDto" @handleEdit="handleEdit(row.Id)" @handleDelete="handleDelete(row)"
                @handleView="handleView(row)" />
        </template>
    </TableView>

    <el-dialog v-model="dialogVisible" :title="AddModifyView==1?'新增角色':AddModifyView==2?'修改角色':'查看角色'" size="default"
        width="700" draggable>
        <div>
            <el-form ref="ruleFormRef" :model="roleDto" :rules="rules" label-width="auto">
                <el-form-item label="角色名称" prop="RoleName">
                    <el-input :readonly="AddModifyView==3" v-model:model-value="roleDto.RoleName"
                        placeholder="请输入角色名称" />
                </el-form-item>
                <el-form-item label="角色描述" prop="Describe">
                    <el-input :readonly="AddModifyView==3" v-model:model-value="roleDto.Describe"
                        :autosize="{ minRows: 2, maxRows:3 }" placeholder="请输入角色描述" type="textarea" />
                </el-form-item>
                <el-form-item label="显示顺序" prop="Order">
                    <el-input-number :readonly="AddModifyView==3" v-model="roleDto.OrderIndex" :min="0" />
                </el-form-item>
    ...
    <el-drawer v-model="drawerVisible" :title="drawerTitle">
        <div>
            <el-input v-model="treeSelectStr" placeholder="菜单/权限名称" style="width: 240px"
                :prefix-icon="Search"></el-input>
            &emsp;<ElButton type="primary">展开</ElButton> <el-checkbox v-model="fatherAndSon" label="父子联动"
                size="large" />
            <el-tree ref="treeRef" :data="treePermissions" :default-checked-keys="treeDefaultList" show-checkbox
                default-expand-all node-key="id" :check-strictly="!fatherAndSon" :highlight-current="true"
...
import { QuerySingleRequest } from '@/api';
import RoleService from '@/api/RoleService';
import { GetSystemRolePagedListRequest, InsertRoleMenuInPut, SystemRoleDto } from '@/api/RoleService/model';
...
const roleDto = ref<SystemRoleDto>({
        Id:'',
        RoleName:'',
        Describe:'',
        IsDefault:false,
    ...
    const getRoleSelect = reactive<GetSystemRolePagedListRequest>({
        RoleName:'',
        PageIndex:1,
        PageSize:10,
    })
...
//分配权限
function handleAssign(row: SystemRoleDto){
    roleApi.handleTreePermissions({Id:row.Id} as QuerySingleRequest).then(res=>{
        drawerTitle.value=`【${row.RoleName}】分配权限`;
        drawerVisible.value=true;
        treePermissions.value=res.menuTreeList;
        treeDefaultList.value=res.roleMenIdList;
        //InsertRoleMenuDto.=row.Id;
    })
}
//分配权限确定
...
function handleDelete(row: SystemRoleDto){
    confirmDelete(() => {
        roleApi.deletRole({ Id: row.Id } as QuerySingleRequest).then((res) => {
            if (res) {
...
function handleView(row: SystemRoleDto){
    roleDto.value=row;
...
const LoadPage = () => {
    roleApi.GetSystemRolePagedList(getRoleSelect).then((res) => {
